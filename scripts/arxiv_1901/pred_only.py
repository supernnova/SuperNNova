...
def get_predictions(model_dir, pred_dir, dataset_path):
    ...
    config = yaml.load(open(Path(model_dir) / "cf.yml", "r"), Loader=yaml.FullLoader)
...
def get_metrics(model_dir, pred_dir):
...
def main(config_path, pred_dir, dataset_path):
    ...
    # Compute metrics
    get_metrics(config["dump_dir"], pred_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument("dataset_path", help="Path to where data is stored in HDF5")
